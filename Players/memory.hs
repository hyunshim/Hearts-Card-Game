...
-- trace ("prev: " ++ show previous_state ++ " cards: " ++ show player_cards) 
...
updateMem :: Memory -> Bool -> [Card] -> String
updateMem memory q_played c_played = show $ memory { 
    queen_played = if (q_played || queen_played memory) then True else False,
    cards_played = sortCards $ cards_played memory ++ c_played
    }

playCard :: PlayFunc
playCard player_id player_cards current_trick previous_state
    | elem (Card Club Two) player_cards = trace ("p_id: " ++ player_id ++ "\t\t\t" ++ show previous_state) ((Card Club Two), mem) 
        -- If C2 in hand --> Play C2

    | (null current_trick) = trace ("p_id: " ++ player_id ++ "\t\t\t" ++ show previous_state) (firstToPlay player_cards memory, mem)
        -- If I'm the first to play --> select card using firstToPlay

    | (null $ cardsOfSuit current_trick_suit player_cards) = trace ("p_id: " ++ player_id ++ "\t\t\t" ++ show previous_state) (noLeadingSuit player_cards memory, mem)
        -- If I don't have a card of the leading suit --> select card using noLeadingSuit

    | not $ null $ cardsOfSuit current_trick_suit player_cards = trace ("p_id: " ++ player_id ++ "\t\t\t" ++ show previous_state) ((highestOfLowest current_trick current_trick_suit player_cards), mem)
        -- If I have a card of the leading suit --> select card using highestOfLowest

    | otherwise = trace ("p_id: " ++ player_id ++ "\t\t\t" ++ show previous_state) ((head $ sortCards player_cards), mem)
        -- Else --> play the first card in hand
        where 
            trick_cards = cardsInTrick current_trick
            current_trick_suit = getSuit (last trick_cards)
            cards_played = getPreviousCards previous_state
            queen_played = if ((Card Spade Queen) `elem` (getPreviousCards previous_state)) then True else False
            players = if (length player_cards == 13 && previous_state == Nothing) then "4" else "2"
            
            memory = getMemory previous_state
            mem = if memory == "" 
                then "Memory {player_count = " ++ players ++ ", queen_played = False, cards_played = []}" 
                else updateMem (read memory :: Memory) queen_played cards_played
