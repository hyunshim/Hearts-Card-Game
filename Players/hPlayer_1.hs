...
playCard player_id player_cards current_trick previous_state
    ...
    | (null current_trick) || (null $ cardsOfSuit current_trick_suit player_cards) =
        if (elem 'H' (show memory)) 
            then (playPointCard head player_cards, heartsPlayed)
            else if (((length $ cardsNotOfSuit Heart $ cardsNotOfSuit Spade player_cards) > 0))
                then ((head $ sortCards $ cardsNotOfSuit Spade player_cards), memory ++ heartsPlayed) 
                else ((head $ sortCards player_cards), memory ++ heartsPlayed) 
    ...
    | otherwise = ((head $ sortCards player_cards), heartsPlayed)
        ...
        where 
            ...
            heartsPlayed = if elem 'H' (show $ getPreviousCards previous_state) then "H" else memory
