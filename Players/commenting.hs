...
playCard _ player_cards current_trick previous_state
    ...
    | (null current_trick || current_trick_suit == Spade) && (Card Spade Queen) `elem` player_cards = (playHeart2 Spade player_cards spades_played trick_cards, mem)
    ...
    | otherwise = ((head $ sortCards player_cards), mem)
        ...
        where 
            ...
            spades_played = sortCards $ (cardsOfSuit Spade trick_cards) ++ (cardsOfSuit Spade all_cards_played)
