...
playPointCard player_cards hearts_played trick_cards
    | length hearts_played == 0 = head $ sortCards $ player_cards
    | return_card /= (Card Club Two) = return_card
    | return_card == (Card Club Two) && length trick_cards == 0 = head $ player_cards
    | otherwise = last $ player_hearts
        where
            return_card = checkSmaller (reverse player_cards) hearts_not_played trick_cards
...
playCard player_id player_cards current_trick previous_state
    ...
    | (null current_trick) || (current_trick_suit == Heart) = trace (trace_str) (playPointCard player_cards hearts_played trick_cards, mem)
    ...
    -- | not $ null $ cardsOfSuit current_trick_suit player_cards = trace (trace_str) ((playPointCard player_cards hearts_played trick_cards), mem)
